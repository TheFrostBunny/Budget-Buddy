            ...
            if (chartType === 'line') {
              return (
                <LineChart data={data} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
                  ...
                  <Tooltip formatter={(value: number) => `${value.toLocaleString()} ${inputCurrency || ''}`} labelFormatter={label => `${t('dashboard.graph.date', 'Dato')}: ${label}`} />
              ...
              return (
                <PieChart>
                  <Pie
                    ...
                    cy="50%"
                    outerRadius={80}
                    label={({ name, value }) => `${name}: ${value.toLocaleString()}${inputCurrency ? ' ' + inputCurrency : ''}`}
                  >
                  ...
                  <Tooltip formatter={(value: number, name: string) => [`${value.toLocaleString()}${inputCurrency ? ' ' + inputCurrency : ''}`, name]} />
                  <Legend />
                </PieChart>
              );
            }
